...
def get_crypto_report():
    try:
        ...
        report =  f"📊 **[셀러유니온] 온체인 브리핑**\n"
        report += f"📅 일시: {today}\n"
        report += f"━━━━━━━━━━━━━━━━━━\n\n"
        
        report += f"💰 **비트코인 시세 (BTC)**\n"
        report += f"└ 현재가: `{upbit_price:,.0f} 원`\n"
        report += f"└ 김치프리미엄: `{kimp:.2f}%` 🇰🇷\n\n"
        
        report += f"🧠 **시장 심리 및 지배력**\n"
        report += f"└ 공포탐욕지수: {fng_value} [{fng_kor}]\n"
        report += f"└ BTC 도미넌스: {btc_dom:.1f}%\n\n"
        
        report += f"🌐 **글로벌 마켓 데이터**\n"
        report += f"└ 전체 시총: ${total_mcap/1e12:.2f}T\n\n"
        
        report += f"━━━━━━━━━━━━━━━━━━\n"
        report += f"📢 **Seller Union Data Center**\n"
        report += f"본 리포트는 실시간 데이터를 바탕으로 자동 생성되었습니다."
